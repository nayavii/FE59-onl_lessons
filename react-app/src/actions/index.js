...
export const addPostSAction = (posts) => ({type: RECEIVED_POSTS, payload: posts});  
...
export const addUserDataAction = (user) => ({type: RECEIVED_USER_DATA, payload: user});
export const addPostByIdAction = (post) => ({type: RECEIVED_POST, payload: post});  // action creator

const URL = "https://studapi.teachmeskills.by"

export const addPostsMiddlewareActions = () => {
  return (dispatch) => {
...
export const registerMiddlewareActions = ({ name, email, password, course_group }) => {
  return (dispatch) => {
...
export const addPostByIdMiddlewareActions = (postId) => {
  return (dispatch) => {
